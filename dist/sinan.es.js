...
function lastIndexOf(array, value, fromIndex) {
  ...
  if (fromIndex !== void 0) {
    ...
    index = index < 0 ? nativeMax$1(length + index, 0) : nativeMin(index, length - 1);
  }
  return value === value ? strictLastIndexOf(array, value, index) : baseFindIndex(array, baseIsNaN, index, true);
}
class Number_extract {
  constructor(option) {
    __publicField(this, "digit", {
      \u4E00: 1,
      \u4E8C: 2,
      \u4E09: 3,
      \u56DB: 4,
    ...
    __publicField(this, "digit_list", Object.keys(this.digit));
    __publicField(this, "digit_unit", {
      \u4E2A: 1,
      \u5341: 10,
      \u767E: 100,
      \u5343: 1e3,
    ...
    __publicField(this, "digit_unit_list");
    __publicField(this, "other_unit_list", []);
    for (const unit of option.measureUnit)
      this.other_unit_list.push(unit);
    this.digit_unit_list = Object.keys(this.digit_unit);
  }
  decimal_detect(query) {
  ...
  unitchar2num(detect_char) {
    detect_char = detect_char.replace("\u96F6", "");
    let idx_y = lastIndexOf(detect_char, "\u4EBF");
    let idx_w = lastIndexOf(detect_char, "\u4E07");
    if (idx_w < idx_y)
      idx_w = -1;
    let [num_y, num_w] = [1e8, 1e4];
    if (idx_y !== -1 && idx_w !== -1) {
      ...
      return +this.unitchar2num(detect_char.substring(0, idx_y)) * num_y + +this.unitchar2num_lower(detect_char.substring(idx_y + 1)) + "";
    } else if (idx_w !== -1) {
  ...
  unitchar2num_lower(s) {
    ...
    if (idx_q !== -1)
      num += this.digit[s.substring(idx_q - 1, idx_q)] * 1e3;
    if (idx_b !== -1)
      num += this.digit[s.substring(idx_b - 1, idx_b)] * 100;
    if (idx_s != -1)
      num += ((_a = this.digit[s.substring(idx_s - 1, idx_s)]) != null ? _a : 1) * 10;
    if (idx_one !== -1 && s.indexOf("\u4E2A") > 0 && indexOf(this.digit_unit_list, s[s.indexOf("\u4E2A") - 1]) === -1) {
      num += (_b = this.digit[s.substring(idx_one - 1, idx_one)]) != null ? _b : 1;
    }
  ...
  fraction_transfer(detect_char) {
    if (detect_char === "\u4E2A")
      return "";
    const re = /([亿万千百十个])/g;
    if (!re.test(detect_char)) {
      detect_char = detect_char.slice(-1);
    }
    let [denominator_char, numerator_char] = detect_char.split("\u5206\u4E4B");
    let fraction_num;
    if (numerator_char !== void 0) {
  ...
  unit_mix_detect(query) {
    var _a;
    const mix_rule = /(?<base_num>\d+(.\d+)?)(?<base_unit>(万亿|千亿|百亿|十亿|亿|千万|百万|十万|万|千|百))(?<res_num>\d+)?/;
    let char_new = query;
    while (char_new.match(mix_rule)) {
      const mix_content = char_new.match(mix_rule);
      const base_num = (mix_content == null ? void 0 : mix_content.groups["base_num"]) || "";
      const base_unit = (mix_content == null ? void 0 : mix_content.groups["base_unit"]) || "";
      const res_num = (_a = mix_content == null ? void 0 : mix_content.groups["res_num"]) != null ? _a : "0";
      const base_unit_num = this.unitchar2num(["\u4E00", base_unit].join(""));
      const mix_num = parseFloat(base_num) * parseFloat(base_unit_num) + parseFloat(res_num) * parseFloat(base_unit_num) * 10 ** -res_num.length;
      char_new = char_new.replace(mix_rule, mix_num + "");
    }
    return char_new;
  }
  integer_detect(query) {
    const num_and_unit_list = this.digit_list.concat(this.digit_unit_list);
...
class Number_rule {
  constructor(option) {
    __publicField(this, "onlyPickMax", false);
    __publicField(this, "measure_dict");
    var _a;
    this.onlyPickMax = (_a = option.onlyPickMax) != null ? _a : this.onlyPickMax;
    this.measure_dict = option.measureDict;
  }
  getStdResult(query) {
    var _a;
    let obj = {};
    const keys2 = Object.keys(this.measure_dict);
    for (const key of keys2) {
      ...
      for (const match of matchArr) {
        ...
        const unit_match_val = (_a = match.groups) == null ? void 0 : _a[key];
        const unit_convert_val = item[unit];
        if (unit_std === void 0) {
          console.warn(`DICT.${key} missed 'std' prop`);
          continue;
        }
        if (unit_match_val !== void 0) {
...
const BASE_DICT = {
  ...
  weight: { \u5343\u514B: 1, kg: 1, \u516C\u65A4: 1, \u65A4: 0.5, \u514B: 1e-3, g: 1e-3, std: "kg" },
  ...
  area: { \u5E73\u65B9\u7C73: 1, \u5E73\u7C73: 1, \u5E73\u65B9\u5343\u7C73: 1e3, \u4EA9: 666, std: "\u5E73\u65B9\u7C73" },
  floor: { \u697C: 1, \u5C42: 1, std: "\u697C" }
};
class CreateParser {
  constructor(option) {
    __publicField(this, "numberExtract");
    __publicField(this, "numberRule");
  ...
  getMeasureDict(option) {
    return (option == null ? void 0 : option.measureDict) ? _merge(cloneDeep(BASE_DICT), (option == null ? void 0 : option.measureDict) || {}) : cloneDeep(BASE_DICT);
  }
  getMeasureUnit(dict) {
    return ["\u5E73\u65B9\u7C73", "\u697C"];
  }
